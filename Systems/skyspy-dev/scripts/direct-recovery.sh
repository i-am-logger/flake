...
# Direct Windows Recovery - No Prompts
echo "=== Starting Windows Recovery (Console Mode) ==="
echo ""
...
TEMP_VARS="$HOME/ovmf_vars_direct.fd"
rm -f "$TEMP_VARS"
dd if="$OVMF_VARS_TEMPLATE" of="$TEMP_VARS" bs=1M 2>/dev/null
chmod 644 "$TEMP_VARS"

echo "BCD Repair Commands for Windows Command Prompt:"
echo "------------------------------------------------"
echo "diskpart"
echo "list disk"
echo "select disk 0"
...
echo "------------------------------------------------"
echo ""
echo "Starting QEMU console mode now..."
echo "Use Ctrl+A then X to exit QEMU when done"
echo ""
...
echo ""
echo "Recovery session ended. Cleaning up..."

# Cleanup
rm -f "$TEMP_VARS"

echo "Done! Test Windows boot with: ./test-windows-select.sh"
